...
export class MACD {
  ...
  update(price: OHLCV): MACDValue {
    ...
    const fastResult = this.fastEMA.update(price);
    const slowResult = this.slowEMA.update(price);

    let macdValue: number | null = null;

    // Check if both results exist and extract their values
    if (fastResult !== null && slowResult !== null) {
      macdValue = fastResult.value! - slowResult.value!;
    }
    ...
    if (macdValue !== null) {
      ...
      const signalResult = this.signalEMA.update(signalPrice);
      
      // Check if signal result exists before accessing .value
      if (signalResult !== null) {
        signalValue = signalResult.value;
      }
    }
