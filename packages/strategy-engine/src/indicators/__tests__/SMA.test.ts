...
describe('SMA (Simple Moving Average)', () => {
  ...
  describe('calculate()', () => {
    ...
    it('should calculate SMA correctly for exact period', () => {
      ...
      expect(results[0].value).toBe(20); // (10+20+30)/3
    ...
    it('should calculate multiple SMA values for longer dataset', () => {
      ...
      expect(results[0].value).toBe(20); // (10+20+30)/3
      expect(results[1].value).toBe(30); // (20+30+40)/3
      expect(results[2].value).toBe(40); // (30+40+50)/3
    });
    ...
    it('should calculate 20-period SMA correctly', () => {
      ...
      // Create 25 days of data with sequential values
      for (let i = 1; i <= 25; i++) {
      ...
      expect(results).toHaveLength(6); // 25 - 20 + 1 = 6
      
      // First SMA should be average of 1-20
      // (10+20+30+...+200)/20 = 2100/20 = 105
  ...
  describe('update()', () => {
    it('should return null when insufficient data', () => {
      ...
      expect(sma.update(createOHLCV(300, new Date()))).toBeNull();
    });

    it('should return SMA value after receiving enough data', () => {
      const sma = new SMA(3);
      ...
      const result = sma.update(createOHLCV(400, new Date()));
      
      expect(result).toBe(300); // (200+300+400)/3
    });
    ...
    it('should maintain sliding window correctly', () => {
      ...
      expect(sma1).toBe(30); // (20+30+40)/3
      expect(sma2).toBe(40); // (30+40+50)/3
      expect(sma3).toBe(50); // (40+50+60)/3
    });
    ...
    it('should work correctly after multiple updates', () => {
      ...
      expect(sma.update(createOHLCV(300, new Date()))).toBe(250); // (200+300)/2
      expect(sma.update(createOHLCV(400, new Date()))).toBe(350); // (300+400)/2
    });
  });
  ...
  describe('getValue()', () => {
    ...
    it('should return current SMA value after enough updates', () => {
      ...
      expect(sma.getValue()).toBe(300); // (200+300+400)/3
    });
    ...
    it('should return same value as last update()', () => {
      ...
      const getValueResult = sma.getValue();
    ...
    it('should return null after calculate() is used', () => {
      ...
      // calculate() doesn't set internal state
      expect(sma.getValue()).toBeNull();
    });
  });
  ...
  describe('reset()', () => {
    ...
    it('should allow starting fresh after reset', () => {
      ...
      expect(sma.update(createOHLCV(100, new Date()))).toBeNull();
      expect(sma.update(createOHLCV(150, new Date()))).toBe(125); // (100+150)/2
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle large datasets', () => {
      ...
      expect(results[0].value).toBeCloseTo(124.5, 1); // Average of 100-149
    });
    ...
    it('should handle decimal values correctly', () => {
      ...
      expect(results[0].value).toBeCloseTo(20.5, 2); // (10.5+20.7+30.3)/3
    });
  });
});